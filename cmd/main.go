...
func main() {
	...
	listOrderUseCase, err := wire.InitializeListOrderUseCase(db)
	if err != nil {
		panic(err)
	}
	...
	listOrderHandler := web.NewListOrderHandler(listOrderUseCase)
	//Inicializar o servidor web
	webserver := webserver.NewWebServer(config.WebServerPort)
	webserver.AddHandler("/order", webOrderHandler.Create)
	webserver.AddHandler("/orders", listOrderHandler.List)
	...
	createOrderService := services.NewOrderService(createOrderUseCase, listOrderUseCase)
	...
	//Servidor GraphQL.
	graph.NewServer(createOrderUseCase, listOrderUseCase)
