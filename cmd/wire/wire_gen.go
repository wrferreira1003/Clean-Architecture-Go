...
func InitializeListOrderUseCase(db *sql.DB) (*usecase.ListOrderUseCase, error) {
	...
	return listOrderUseCase, nil
}

// wire.go:

// set de provedores seria tudo que vamos utilizar no projeto
var OrderSet = wire.NewSet(database.NewOrderRepository, wire.Bind(new(repository.OrderRepositoryInterface), new(*database.OrderRepository)), events.NewEventDispatcher, wire.Bind(new(events.EventDispatcherInterface), new(*events.EventDispatcher)), events.NewOrderCreatedEvent, usecase.NewCreateOrderUseCase, usecase.NewListOrderUseCase)
